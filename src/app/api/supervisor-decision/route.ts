...
function buildSupervisorPrompt(
	...
	return `You are a supervisor agent managing a group chat discussion named "${context.groupName}". Your role is to decide who should speak next based on the conversation context and their expertise.
...
{
  "nextSpeaker": ["member_id_1", "member_id_2"]
}

Rules:
- Always return nextSpeaker as an array of member ids from the GroupMembers list
- If DM needed, include the DM sender in the nextSpeaker array
- If a member's expertise is needed based on the conversation topic, include that member
- If the conversation seems complete, return empty array. DM needed should not be considered as a conversation complete, and the DM sender need to be included in the nextSpeaker array.
...
function validateDecision(
	...
	for (const speaker of decision.nextSpeaker) {
		if (!members.find((m) => m.name === speaker || m.id === speaker)) {
